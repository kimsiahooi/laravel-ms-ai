...
import { Ban, LoaderCircle, PackagePlus, Printer } from 'lucide-react';
...
export default function SalesReturnShow() {
    ...
    return <SalesReturnDetail ret={ret} warehouses={warehouses} />;
}
...
function SalesReturnDetail({
    ...
    return (
        ...
        >
            ...
            <DetailHeader
                title={`Sales return #${ret.id}`}
                status={{ status: ret.status, label: ret.status_label }}
                description={ret.customer ? `For ${ret.customer}` : undefined}
                actions={
                    <>
                        {pending ? (
                            <>
