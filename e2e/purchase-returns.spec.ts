...
test('purchase returns: create with a line item, then complete it', async ({
    page,
}) => {
    const errors = collectErrors(page);
    await login(page);

    await page.goto('/e2e/purchase-returns');
    ...
    await dialog
        .getByRole('button', { name: /create purchase return/i })
        .click();
    await expect(page.getByText('Purchase return created.')).toBeVisible();

    // Complete it from the main warehouse (raw materials are stocked there).
    await page
    ...
    await completeDialog.getByRole('combobox', { name: 'Return from' }).click();
    await page.getByRole('option', { name: /kuala lumpur/i }).click();
    await completeDialog
        .getByRole('button', { name: 'Complete return' })
        .click();
    await expect(page.getByText('Purchase return completed.')).toBeVisible();
